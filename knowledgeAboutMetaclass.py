...
# 你可以从这里获取什么？
# 1.
# 也许你在阅读别人的代码的时候碰到过metaclass，那你可以参考这里的介绍。
# 2.
# 或许你需要设计一些底层的库，也许metaclass能帮你简化你的设计（也有可能复杂化：）
# 3.
# 也许你在了解metaclass的相关知识之后，你对python的类的一些机制会更了解。
# 4.
# more......
#
# 二
...
#
#
# class defines the behavior of certain objects,
#
#
# a
# metaclass
...
# accepting
# arguments
# for name, bases, and dict.Upon class creation, the callable is used instead of the built- in type().New in version 2.2.(所以有了上面介绍的分别用类或者函数的方法）
#
...
# exists, it is used.
#     Otherwise,
# if there is at least one base
#
#
# class , its metaclass is used (this looks for a __class__ attribute first and if not found, uses its type).
#
#
# Otherwise,
# if a global variable named __metaclass__ exists, it is used.
# Otherwise, the
# old - style, classic
# metaclass(types.ClassType) is used.
#
# 这个查找顺序也比较好懂，而且利用这个顺序的话，如果是old - style类的话，可以在某个需要的模块里面指定全局变量
# __metaclass__ = type就能把所有的old - style
# 变成
# new - style的类了。(这是其中一种trick)
#
# 四
# 例子
...
# 根据类名字，如果是以
# 'Abc'
# 开头的就给它加上ma和mb的方法(这里的条件只是一种简单的例子假设了，实际应用上
# 可能更复杂）, 如果不是
# 'Abc'
# 开头的类就不加.这样就可以打到动态添加方法的效果了。其实，你也可以将需要动态
...
# them, you
# don
# 't (the people who actually need them know with certainty that they need them, and don'
# t
# need
# an
# explanation
...
# 2.
# metaclass的优势在于它的动态性和可描述性（比如上面例子中的self.delegate.__getitem__(i)
# 这样的代码，它
...
#
# 4.
# 在python中我们为什么要修改class？那当然是为了改变它的行为，或者为了创
...
#
# 5.
# python的成功之处在于它无处不在的namespace(就是那个__dict__，其意义可以
# 参考SICP第一章的environment模型，对计算理论感兴趣的也可以用lambda演算来
# 解释)，而且函数又是first
# class对象，又强化了interface的使用。我们知道了
# metaclass->class ->instance的关系，又知道了对象的方法是放在类里的(请详细
# 考察python查找一个方法的流程)，那么用python实现各种设计模式就比较简单了。
#
#
# 6.
# metaclass不会使程序变晦涩，前提是了解了metaclass的固有存在，许多教程的
# 问题就在于它没有告诉读者metaclass的存在，而是借助某些其他语言(比如c + +)
# 的
...
# 在__init__中控制类生成的过程有一点要注意：在__init__()
# 的最后一个参
# 数(attrs)
# 中，对于类中定义的函数类型的属性，比如：
# def abc(self):
